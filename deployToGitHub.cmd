...
echo Clearing files
rmdir /s "%APPVEYOR_BUILD_FOLDER%\temp_dist*"
mkdir "%APPVEYOR_BUILD_FOLDER%\temp_dist"

echo Copying new files to dist repo
copy -rf "%APPVEYOR_BUILD_FOLDER%\dist\*" "%APPVEYOR_BUILD_FOLDER%\temp_dist\"

echo Re-apply GIT settings for dist repo
xcopy /h "%APPVEYOR_BUILD_FOLDER%\temp_dist_git_settings\.git*" "%APPVEYOR_BUILD_FOLDER%\temp_dist"
