namespace Fifthweek.Api.Posts.Tests.Commands
{
    ...
    public class PostImageCommandHandlerTests
    {
        ...
        private static readonly PostImageCommand Command = new PostImageCommand(Requester, PostId, QueueId, FileId, Comment, ScheduleDate, IsQueued);
        ...
        {
            await this.target.HandleAsync(new PostImageCommand(Requester.Unauthenticated, PostId, QueueId, FileId, Comment, ScheduleDate, IsQueued));
        }
        ...
        public async Task WhenAllowedToPost_ItShouldPostToCollection()
        {
            this.postToCollectionDbStatement.Setup(
                _ => _.ExecuteAsync(PostId, QueueId, Comment, ScheduleDate, IsQueued, FileId, true, It.IsAny<DateTime>()))
